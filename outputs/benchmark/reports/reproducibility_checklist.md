...
- [x] `gemma4:latest` digest recorded BEFORE the full run
      ...
      Ollama 0.20.0, Q4_K_M, 8.0B parameters. `fail_on_digest_change: true`.
- [x] Fixed Gemma 4 prompt, SHA-256 in the manifest; zero-shot, no BIA
      examples; unchanged after the smoke test.
- [x] Fixed seeds: benchmark/bootstrap/Gemma = 42; extractor (paper) = 13.
- [x] External system commits recorded (DptOIE `1a5ef708`,
      Multi2OIE `4a73a3c3`, dptoie-neural/PortNOIE `770f29fe`) + SHA-256 of
      binary artifacts (`patches/*.md`).
- [x] OS, Python, PyTorch, Transformers, Stanza, CUDA, CPU, GPU, and RAM
      versions in `environment.json`/`manifest.json`.
...
- [x] Evaluation re-runnable from saved predictions
      (`scripts/evaluate_benchmark.py`) without re-extraction; determinism
      covered by tests (`tests/test_determinism.py`).
...
- [x] Sentence-level paired bootstrap, 10,000 resamples, seed 42, 95%
      percentile CIs; between-system differences on the same resamples.
